...
openssl req -new -x509 -days 365 -keyout ca.key -out ca.pem \
  -subj "/C=DE/ST=NRW/L=Earth/O=Random Company/OU=IT/CN=www.random.com/emailAddress=test@test.com" -passout pass:$PRIVKEY

echo "make server cert"
openssl req -new -nodes -x509 -out server.pem -keyout server.key -days 3650 \
  -subj "/C=DE/ST=NRW/L=Earth/O=Random Company/OU=IT/CN=www.random.com/emailAddress=${EMAIL}"
...
openssl x509 -req -days 365 -in client.req -CA ca.pem -CAkey ca.key \
  -passin pass:$PRIVKEY -out client.pem

openssl x509 -extfile ../openssl.conf -extensions ssl_client -req -days 365 \
  -in client.req -CA ca.pem -CAkey ca.key -passin pass:$PRIVKEY -out client.pem
